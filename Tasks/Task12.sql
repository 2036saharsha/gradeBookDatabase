...
AssignmentSum AS (
    SELECT
        e.STUDENT_ID,
        SUM(CASE WHEN a.GRADE = MIN_GRADE THEN 100 ELSE COALESCE(a.GRADE, 0) END) AS ASSIGNMENT_SUM
    FROM
        ENROLLMENT e
        ...
        CROSS JOIN (
            SELECT MIN(COALESCE(a.GRADE, 0)) AS MIN_GRADE
            FROM ENROLLMENT e
            LEFT JOIN ASSIGNMENT a ON e.GRADE_ID = a.GRADE_ID
            JOIN COURSE_SCHEDULE cs ON e.SCHEDULE_ID = cs.SCHEDULE_ID
            JOIN COURSE c ON cs.COURSE_NUM = c.COURSE_NUM
            WHERE e.STUDENT_ID = 201
